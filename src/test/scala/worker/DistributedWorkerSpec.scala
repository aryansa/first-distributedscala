...
object DistributedWorkerSpec {
  ...
  class FlakyWorkExecutor extends Actor with ActorLogging {
    ...
    def receive = {
      case WorkExecutor.DoWork(n: BigInt) =>
        i += 1
        if (i == 3) {
...
class DistributedWorkerSpec(_system: ActorSystem)
  ...
  "Distributed workers" should "perform work and publish results" in {
    ...
    frontend ! Work("1", new Part(0,10))
    expectMsg("ok-1")
    within(10.seconds) {
      awaitAssert {
        results.expectMsgType[WorkResult].workId should be("0 1 2 3 5 7")
      }
    }
    ...
    for (n <- 2 to 5) {
      frontend ! Work(n.toString, new Part(10*n,10))
      expectMsg(s"ok-$n")
    }
    system.log.info("5 work items sent")

    results.within(20.seconds) {
      val ids = results.receiveN(4).map { case WorkResult(workId, _) => workId }
      // nothing lost, and no duplicates
      ids.toVector.map(_.toInt).sorted should be((2 to 5).toVector)
    }
